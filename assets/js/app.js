const projectId = window.APP_PROJECT_ID;
const canEditProject = window.APP_CAN_EDIT !== false;
const phases = ['L1','L2','L3'];
let devices = [], circuits = [], plan = [], activeCircuit = null;
const esc = v => String(v ?? '').replace(/[&<>"']/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[c]));
const fmtW = v => `${Math.round(Number(v||0)).toLocaleString('de-DE')} W`;
...
async function savePatch(item, data){ if(!canEditProject) return; await sendJson(`${api.items}&id=${item.id}`,'PATCH',data); await loadAll(); }
...
function renderPlanCard(i){ return `<article class="plan-card" draggable="${canEditProject ? 'true' : 'false'}" data-id="${i.id}"><div class="d-flex justify-content-between gap-2"><div><strong>${esc(i.brand?i.brand+' · '+i.name:i.name)}</strong><div class="small-muted">Anzahl: ${i.quantity} · ${fmtW(itemWatts(i))}</div>${i.remarks?`<div class="plan-remarks mt-1">${esc(i.remarks)}</div>`:''}</div>${canEditProject ? `<button class="btn btn-sm btn-outline-danger" onclick="removeItem(${i.id})">×</button>` : ''}</div><div class="d-flex justify-content-between mt-2"><span class="badge text-bg-dark">${i.phase}</span><span class="fw-semibold">${fmtA(itemAmps(i))}</span></div></article>`; }
function renderRows(){ els.planRows.innerHTML=plan.length?plan.map(i=>`<tr><td><strong>${esc(i.name)}</strong><div class="small-muted">${esc(i.category||'-')}</div></td><td>${esc(i.brand||'-')}</td><td><input class="form-control form-control-sm plan-quantity-input" type="number" min="1" value="${i.quantity}" onchange="updateQuantity(${i.id},this.value)" ${canEditProject ? '' : 'disabled'}></td><td>${esc(circuitName(i.circuit_id))}</td><td>${i.phase}</td><td>${fmtW(itemWatts(i))}</td><td>${fmtA(itemAmps(i))}</td><td><textarea class="form-control form-control-sm plan-remarks-input" onchange="updateRemarks(${i.id},this.value)" ${canEditProject ? '' : 'disabled'}>${esc(i.remarks||'')}</textarea></td><td class="text-end">${canEditProject ? `<button class="btn btn-sm btn-outline-danger" onclick="removeItem(${i.id})">Löschen</button>` : ''}</td></tr>`).join(''):'<tr><td colspan="9" class="text-center text-muted py-4">Noch keine Geräte im Plan.</td></tr>'; }
...
async function createCircuit(){
  if(!canEditProject){ AppUI.warning('Archivierte oder schreibgeschützte Projekte können nicht bearbeitet werden.'); return; }
...
els.deleteCircuit.addEventListener('click',async()=>{ if(!canEditProject){ AppUI.warning('Archivierte oder schreibgeschützte Projekte können nicht bearbeitet werden.'); return; } if(!(await AppUI.confirm('Stromkreis löschen? Zugeordnete Planpositionen werden entfernt.', {title:'Stromkreis löschen', confirmText:'Löschen'})))return; const r=await fetch(`${api.circuits}&id=${activeCircuit}`,{method:'DELETE',credentials:'same-origin'}); const res=await r.json(); if(!r.ok){ AppUI.error(res.error); return; } activeCircuit=null; await loadAll(); });
